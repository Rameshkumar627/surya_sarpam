...
VOUCHER_INFO = [('draft', 'Draft'), ('paid', 'Paid')]
...
class Voucher(surya.Sarpam):
    ...
    voucher_progress = fields.Selection(selection=VOUCHER_INFO, string="Voucher Progress", default="draft")
    ...
    paid_on = fields.Date(string='Paid On', readonly=True)
    paid_by = fields.Many2one(comodel_name='hr.employee', string='Paid By', readonly=True)
    ...
    def trigger_paid(self):
        data = {'paid_on': datetime.now().strftime("%Y-%m-%d"),
                'paid_by': self.env.user.id,
                'voucher_progress': 'paid'}
        self.write(data)
